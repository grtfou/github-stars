"""
@date       180312 - Enhanced doc format
...
def _get_conf():
    ...
    if 'DEFAULT' in config:
        # ex. gvanrossum
        output['target_owner'] = config['DEFAULT']['TARGET_OWNER']
        # ex. README.md
        output['output_filename'] = config['DEFAULT']['OUTPUT_FILENAME']
        # ex. grtfou/my-stars.git
        output['readme_link'] = config['DEFAULT']['README_LINK']
    else:
        raise ValueError
    return output
...
async def fetch(client, conf, page=1):
    """Get starred list.
    Other URL arguments:
      * page: ex. 3
      * per_page: ex. 100
    ...
    url = '{}/users/{}/starred?page={}&per_page=100'.format(
        API_URL, conf['target_owner'], page)
    header = {
...
async def main(loop, conf):
    ...
    async with aiohttp.ClientSession(loop=loop) as client:
        ...
        while True:
            try:
                json_repo = await fetch(client, conf, page)
    ...
    with open(conf['output_filename'], 'w') as ffi:
        # project description
