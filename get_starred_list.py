...
def _get_conf():
    ...
    if 'DEFAULT' in config:
        # ex. gvanrossum
        output['target_account'] = config['DEFAULT']['TARGET_ACCOUT']
        # ex. README.md
...
async def fetch(client, conf, page=1):
    ...
    url = '{}/users/{}/starred?page={}&per_page=100'.format(
        API_URL, conf['target_account'], page)
    header = {
...
async def main(loop, conf):
    ...
    with open(conf['output_filename'], 'w') as ffi:
        # project description
        ffi.write("# my-stars\n")
        ffi.write("List someone Github stars on README.md\n\n")
